"""PDF export using WeasyPrint — port of services/pdfExport.ts (Puppeteer → WeasyPrint)."""
...
def _render_html(assessment: AssessmentResult) -> str:
    ts = assessment.technicalSummary
    op = assessment.gridOperator
    from datetime import datetime, timezone

    try:
        dt = datetime.fromisoformat(assessment.generatedAt.replace("Z", "+00:00"))
        date_str = dt.strftime("%d.%m.%Y")
    except Exception:
        date_str = assessment.generatedAt

    grid_str = f"Netzbetreiber: {op.name} · {op.city} · {op.state}" if op else ""

    claims_html = "".join(
        f"""<div class="row">
  {_STATUS_BADGE.get(c.status.value, _STATUS_BADGE["unclear"])}
  <div>
    ...
    <div class="row-ref">{c.sourceRef}</div>
        ...
        for c in assessment.regulatoryClaims[:8]
    )

    subsidies_html = "".join(
        f"""<div class="sc">
  <div style="display:flex;justify-content:space-between;align-items:center">
    <div class="sc-name">{s.name}</div>{_STATUS_BADGE.get(s.status.value, "")}
  </div>
  <div class="sc-amount">{s.amount}</div>
  <div class="sc-desc">{s.description}</div>
</div>"""
        for s in assessment.subsidies
    )

    open_points_html = "".join(
        f'<div class="oq">° {p}</div>' for p in assessment.openPoints
    )
    steps_html = "".join(
        f"<li>{s.text}</li>" for s in assessment.nextSteps
    )
...
<div class="grid4">
  ...
  <div class="card"><div class="card-label">Jahresertrag</div><div class="card-value">{ts.annualKwh:,} kWh</div><div class="card-sub">PVGIS ±15%</div></div>
...
<div class="footer">
  ...
  Plankton PV Assistant · {date_str}
</div>
</body>
</html>"""


def generate_pdf(assessment: AssessmentResult) -> bytes:
    """Render assessment as A4 PDF using WeasyPrint (no browser required)."""
    try:
        from weasyprint import HTML  # type: ignore
    except ImportError as exc:
        raise RuntimeError(
            "weasyprint not installed — run: pip install weasyprint\n"
            "Also ensure system libs: brew install pango cairo"
        ) from exc

    html_content = _render_html(assessment)
